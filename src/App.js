...
// 리듀서
import cartSlice from './features/2cart/cartSlice.js';
import userSlice from './features/3asyncthunk/userSlice.js';
import productSlice from './features/3asyncthunk/productSlice.js';
// 디스플레이
import Prac from './features/1practice/Prac.js'
import Cart from './features/2cart/Cart.js'
import User from './features/3asyncthunk/User.js'
import Pokemon from './features/4redux-toolkit-query/ReduxToolkitQuery.js'
// 리덕스툴킷쿼리 사용공부 
import {pokemonApi} from './features/4redux-toolkit-query/pokemonApi.js'

const store = configureStore({
    reducer:{
      cart: cartSlice,    // import하는 컴포넌트에서 export default userSlice.reducer 적을시
      user: userSlice,             
      product: productSlice,
      [pokemonApi.reducerPath]: pokemonApi.reducer, // RTK query 
    },
...
// redux tool kit   리덕스 툴킷은  기존 리덕스를 대체하는 더 편리한 새로운 리덕스 
// 장점. 간편해짐, 몇몇 미들웨어가 포함되있음(thunk), state의immutable불변성
// npm i @reduxjs/toolkit    리덕스 툴킷 사용을 위해
// npm i react-redux    <Provider>, useSelector, useDispatch 사용을 위해 
// app폴더 생성 store.js파일 생성 (이 한파일에만 저장할것을 권장) 그곳에서 configureStore을 셋팅 뒤 import해옴  여기선 그냥 app.js에서 넣어 보여주겠음. 
/* 
